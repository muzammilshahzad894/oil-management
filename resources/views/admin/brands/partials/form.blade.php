<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="number" class="form-control @error('quantity') is-invalid @enderror" id="quantity" name="quantity" value="{{ old('quantity', $brand->quantity ?? '') }}" min="0">
        @error('quantity')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
...
<div class="row"> 
    <div class="col-md-6 mb-3">
        ...
        <input type="number" step="any" min="0" class="form-control @error('cost_price') is-invalid @enderror" id="cost_price" name="cost_price" value="{{ old('cost_price', isset($brand) ? $brand->cost_price : '') }}">
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="number" step="any" min="0" class="form-control @error('sale_price') is-invalid @enderror" id="sale_price" name="sale_price" value="{{ old('sale_price', isset($brand) ? $brand->sale_price : '') }}">
        <small class="text-muted">When creating a sale, amount is calculated as sale price × quantity. You can still change the amount on the sale form.</small>
        @error('sale_price')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
